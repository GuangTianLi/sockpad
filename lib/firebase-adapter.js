...
firepad.FirebaseAdapter = (function (global) {
  ...
  function FirebaseAdapter (socket, userId, userColor, roomId) {
    this.ref_ = socket;
    this.ready_ = false;
    this.roomId_ = roomId;
  ...
  FirebaseAdapter.prototype.dispose = function() {
    ...
    if (this.userRef_) {
      ...
      this.ref_.emit(this.userRef_, {
        point: this.roomId_ + '/' + this.userId_ + '/cursor',
        d: {}
      });
      this.ref_.emit(this.userRef_, {
        point: this.roomId_ + '/' +this.userId_ + '/color',
        d: ''
      });
    }
  ...
  FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    function doTransaction(revisionId, revisionData) {
      self.ref_.emit("history_add", {
        point: self.roomId_ + '/history/' + revisionId,
        d: revisionData
      });
  ...
  FirebaseAdapter.prototype.monitorCursors_ = function() {
    ...
    function childChanged(childSnap) {
      var userId = childSnap.userId;
      var userCursor = childSnap.cursor;
      var userColor = childSnap.color;
      self.trigger('cursor', userId, userCursor, userColor);
    }

    this.ref_.on("users_changed", childChanged);
    // this.firebaseOn_(usersRef, 'child_added', childChanged);
    // this.firebaseOn_(usersRef, 'child_changed', childChanged);

    this.ref_.on("user_left", function(data) {
  ...
  FirebaseAdapter.prototype.monitorHistory_ = function() {
    ...
    this.ref_.emit("last_checkpoint", {
      point: this.roomId_ + '/checkpoint'
    }, function(s) {
      if (self.zombie_) { return; } // just in case we were cleaned up before we got the checkpoint data.
      var revisionId = s.revisionId,  op = s.op, author = s.author;
  ...
  FirebaseAdapter.prototype.monitorHistoryStartingAt_ = function(revision) {
    ...
    setTimeout(function() {
      self.ref_.on("history_added", function(revisionSnapshots) {
        for(var i = 0; i < revisionSnapshots.length; i++) {
          var revisionId = revisionSnapshots[i].revisionId;
          self.pendingReceivedRevisions_[revisionId] = revisionSnapshots[i].d;
          if (self.ready_) {
            self.handlePendingReceivedRevisions_();
          }
        }
      });
      self.ref_.emit("history", {
        point: self.roomId_ + '/history',
        revision: revision
      },function(data) {
